...
Order StrategyTrendFollowStateRun::get_market_buy_spot_order_by_symbol_and_quantity(const std::string& symbol, double quantity)
{
...
Order StrategyTrendFollowStateRun::get_market_sell_spot_order_by_symbol_and_quantity(const std::string& symbol, double quantity)
{
    double round_up_quantity = m_instrument->get_round_up_quantity(quantity);
...
void StrategyTrendFollowStateRun::close_far_orders(double price)
{
    auto task = task_close_far_orders(price);
    task.start_running_on(m_event_base);
}

Task<void> StrategyTrendFollowStateRun::task_close_far_orders(double price)
{
    ...
    for (auto& [order_id, order] : m_open_orders)
    {
        ...
        {
            spdlog::info("StrategyTrendFollowStateRun - close_far_orders: cancel order at price: {}, distance: {}, price: {}", order.price, price_distance, price);
...
void StrategyTrendFollowStateRun::quote_block_orders_at_price(double price)
{
    // MeasureTime t("StrategyTrendFollowStateRun - quote_block", MeasureUnit::MICROSECOND);
    ...
    //     spdlog::info("StrategyTrendFollowStateRun - quote_block_orders_at_price: applying momentum multiplier: {}", mom_mult);
    //     bid_price_gap *= mom_mult;
    //     ask_price_gap *= mom_mult;
    // }

    // spdlog::info("StrategyTrendFollowStateRun - quote_block_orders_at_price: price: {}, bid_gap: {}, ask_gap: {}, inventory: {}, mode: {}",
    ...
    for (size_t i = 1; i <= m_config.orders_each_side_per_block; i++)
    {
        ...
        if (std::abs(buy_price - price) <= m_config.price_step_between_blocks)
        {
            ...
            {
                spdlog::info("StrategyTrendFollowStateRun - stop quoting buy orders at price: {}, m_inventory + i: {}, num_of_limit_orders: {}", buy_price, m_inventory + i, num_of_limit_orders);
                break;
            }
    ...
    for (size_t i = 1; i <= m_config.orders_each_side_per_block; i++)
    {
        ...
        if (std::abs(sell_price - price) <= m_config.price_step_between_blocks)
        {
            ...
            if (m_inventory < 0 && -m_inventory + i > num_of_limit_orders)
            {
                spdlog::info("StrategyTrendFollowStateRun - stop quoting sell orders at price: {}, -m_inventory + i: {}, num_of_limit_orders: {}", sell_price, -m_inventory + i, num_of_limit_orders);
                break;
            }
...
void StrategyTrendFollowStateRun::handle_order_book_snapshot(OrderBookSnapShot* snapshot)
{
    ...
    //     spdlog::info("StrategyTrendFollowStateRun - handle_order_book_snapshot: mid: {}, r: {}, z: {}, mode: {}",
