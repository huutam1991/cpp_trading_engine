...
DataFeedBinanceAggTrade::DataFeedBinanceAggTrade(const std::string& symbol):
    m_symbol(symbol)
{
}

DataFeedBinanceAggTrade::~DataFeedBinanceAggTrade()
{
    ADD_LOG("~DataFeedBinanceAggTrade, " << m_symbol);
}

void DataFeedBinanceAggTrade::init()
{
    ...
    m_websocket->on_close([this](websocket::close_code close_code)
    {
        ...
        {
            // Re-start
            ADD_LOG("Re-start DataFeedBinanceAggTrade");
            this->start();
        }
    });
...
void DataFeedBinanceAggTrade::set_call_back(std::function<void(const std::string& symbol, Json& payload)> call_back)
{
    m_on_callback = call_back;
}

bool DataFeedBinanceAggTrade::standardize_data(const string& buffer, Json& data)
{
