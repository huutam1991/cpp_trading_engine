...
std::mutex DataFeedBinanceFutures::m_df_bf_mutex;

DataFeedBinanceFutures::DataFeedBinanceFutures() : DataFeed(BINANCE_COMMON)
{
    m_url = BINANCE_FUTURES_URL;
    m_port = BINANCE_FUTURES_PORT;
    m_binance_ws_url  = BINANCE_FUTURES_WS_URL;
    m_binance_ws_port = BINANCE_FUTURES_WS_PORT;
}

/*void DataFeedBinanceFutures::check_ws_url_base_on_back_testing()
{
    m_url = BINANCE_FUTURES_URL;
    m_port = BINANCE_FUTURES_PORT;
    m_binance_ws_url  = BINANCE_FUTURES_WS_URL;
...
size_t DataFeedBinanceFutures::get_stream_id_count()
{
    static int stream_id_count = 0;
    std::unique_lock lock(m_df_bf_mutex);

    return ++stream_id_count;
}

void DataFeedBinanceFutures::init()
{
    ...
    m_websocket->on_close([this](websocket::close_code close_code)
    {
        ...
        {
            // Re-start
            ADD_LOG("Re-start DataFeedBinanceFutures");
            this->start();
        }
    });
