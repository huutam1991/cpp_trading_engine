...
DataFeedBinanceFuturesBookTicker::DataFeedBinanceFuturesBookTicker(const std::string& symbol):
    m_symbol(symbol)
{
}

DataFeedBinanceFuturesBookTicker::~DataFeedBinanceFuturesBookTicker()
{
    ADD_LOG("~DataFeedBinanceFuturesBookTicker, " << m_symbol);
}

void DataFeedBinanceFuturesBookTicker::init()
{
    ...
    m_websocket->on_connect([this](WebsocketClientHandle& ws)
    {
        ...
        /*Json params = Json::create_array();
        for (std::string& symbol : m_symbols)
        {
            STRING_LOWER_CASE(symbol);
            params.push_back(symbol + "@bookTicker");
        }*/
    ...
    m_websocket->on_close([this](websocket::close_code close_code)
    {
        ...
        {
            // Re-start
            ADD_LOG("Re-start DataFeedBinanceFuturesBookTicker");
            this->start();
        }
    });
...
void DataFeedBinanceFuturesBookTicker::set_call_back(std::function<void(const std::string& symbol, Json& ticker)> call_back)
{
    m_on_callback = call_back;
}

bool DataFeedBinanceFuturesBookTicker::standardize_data(const std::string& data, Json& ticker)
{
    Json book_ticker = Json::parse(data);
    if (book_ticker.has_field("e") && (std::string&&)book_ticker["e"] == "bookTicker")
    {
        // symbol
        ticker["s"] = BINANCE_FUTURES_ABBREVIATION_NAME + "#" + (std::string&&)book_ticker["s"];
        // event name
        ticker["e"] = book_ticker["e"];
        // event time
        ticker["E"] = stoll(book_ticker["E"].get_string_value());
