...
APIHandlerBinanceCancelOrder::APIHandlerBinanceCancelOrder(HttpRequest* request) : APIHandlerBinance(request)
{
...
Json APIHandlerBinanceCancelOrder::send_cancel_order_request(const std::string& symbol, long orderId)
{
    Json query_json;
    ...
    return send_binance_request("/api/v3/order", query_string, RequestMethod::DELETE);
}

Json APIHandlerBinanceCancelOrder::handle_internal_request(const std::string& symbol, long orderId, const std::string user_id)
{
    // thread safe
    static std::mutex internal_request_mutex;
    std::unique_lock lock(internal_request_mutex);

    APIHandlerBinanceCancelOrder api_handle(nullptr);
...
HttpResponse APIHandlerBinanceCancelOrder::child_handle()
{
    // /api/v3/order?symbol=BNBUSDT&orderId=4322263603
