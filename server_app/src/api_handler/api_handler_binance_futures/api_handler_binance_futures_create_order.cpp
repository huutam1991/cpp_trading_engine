...
APIHandlerBinanceFuturesCreateOrder::APIHandlerBinanceFuturesCreateOrder(HttpRequest* request) : APIHandlerBinanceFutures(request)
{
    m_need_check_authentication = true;
    m_need_check_none_source = true;
}

Json APIHandlerBinanceFuturesCreateOrder::handle_internal_request(Json& query_json)
{
    // thread safe
    ...
    APIHandlerBinanceFuturesCreateOrder api_handle(nullptr);
...
HttpResponse APIHandlerBinanceFuturesCreateOrder::child_handle()
{
...
void APIHandlerBinanceFuturesCreateOrder::handle_query_json(Json& query_json)
{
    // Remove field 'token'
    ...
    // int lot_size = m_url == BINANCE_FUTURES_URL ?
    //     BinanceUtils::instance().get_lot_size_by_symbol_futures(symbol):
    //     BinanceUtils::instance().get_lot_size_by_symbol_test_net(symbol);

    // // Format price
    // size_t round_up_price = m_url == BINANCE_FUTURES_URL ?
    //     BinanceUtils::instance().get_round_up_price_by_symbol_futures(symbol):
    ...
    if (query_json.has_field("positionSide"))
        query_json["positionSide"].set_is_string_format(false);

}

Json APIHandlerBinanceFuturesCreateOrder::send_new_order_request(Json& query_json)
{
    handle_query_json(query_json);
    std::string query_str = this->get_query_string(query_json);

    return send_binance_request("/fapi/v1/order", query_str, RequestMethod::POST);
}

std::string APIHandlerBinanceFuturesCreateOrder::get_query_string(Json& query_json)
{
    ...
    return m_request->get_query_string_from_query_json(query_json) +
           "&type=" + type +
           (type == "LIMIT" ? "&timeInForce=GTC" : ""); // timeInForce=GTC is only for LIMIT
}

Json APIHandlerBinanceFuturesCreateOrder::handle_send_order_request(Json& query_json)
{
    Json response;
    Json order_data = send_new_order_request(query_json);
    ADD_LOG("order_data = " << order_data.get_string_value());
