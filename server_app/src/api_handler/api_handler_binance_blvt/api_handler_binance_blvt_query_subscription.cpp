...
APIHandlerBinanceBLVTQuerySubscripton::APIHandlerBinanceBLVTQuerySubscripton(HttpRequest* request) : APIHandlerBinanceBLVT(request)
{
    m_need_check_authentication = true;
    m_need_check_none_source = true;
}

Json APIHandlerBinanceBLVTQuerySubscripton::handle_internal_request(Json& query_json)
{
    // thread safe
    ...
    APIHandlerBinanceBLVTQuerySubscripton api_handle(nullptr);
...
HttpResponse APIHandlerBinanceBLVTQuerySubscripton::child_handle()
{
...
void APIHandlerBinanceBLVTQuerySubscripton::handle_query_json(Json& query_json)
{
    // Remove field 'token'
...
Json APIHandlerBinanceBLVTQuerySubscripton::send_new_request(Json& query_json)
{
    //this->handle_query_json(query_json);
    std::string query_str = this->get_query_string(query_json);

    return send_binance_request("/sapi/v1/blvt/subscribe/record", query_str, RequestMethod::GET);
}

std::string APIHandlerBinanceBLVTQuerySubscripton::get_query_string(Json& query_json)
{
    Json query;
...
Json APIHandlerBinanceBLVTQuerySubscripton::handle_binance_request(Json& query_json)
{
    Json response;
    ...
    else
    {
        ...
        response["data"] = res;
        response["msg"] = "Query Subscription Record";
...
void APIHandlerBinanceBLVTQuerySubscripton::standardize_response(Json& response)
{
    ...
    if (response.has_field("totalCharge"))
    {
        response["amount"] = response["totalCharge"];
    }
    ...
    response.remove_field("totalCharge");
