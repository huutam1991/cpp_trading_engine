...
APIHandlerBinanceBLVTSubscribe::APIHandlerBinanceBLVTSubscribe(HttpRequest* request) : APIHandlerBinanceBLVT(request)
{
    m_need_check_authentication = true;
    m_need_check_none_source = true;
}

Json APIHandlerBinanceBLVTSubscribe::handle_internal_request(Json& query_json)
{
    // thread safe
    ...
    APIHandlerBinanceBLVTSubscribe api_handle(nullptr);
...
HttpResponse APIHandlerBinanceBLVTSubscribe::child_handle()
{
    Json query_json = m_request->get_query_json();

    const std::string missing_param = m_request->check_missing_params({"symbol", "amount"});
...
void APIHandlerBinanceBLVTSubscribe::handle_query_json(Json& query_json)
{
    // Remove field 'token'
...
Json APIHandlerBinanceBLVTSubscribe::send_new_request(Json& query_json)
{
    //this->handle_query_json(query_json);
    std::string query_str = this->get_query_string(query_json);

    return send_binance_request("/sapi/v1/blvt/subscribe", query_str, RequestMethod::POST);
}

std::string APIHandlerBinanceBLVTSubscribe::get_query_string(Json& query_json)
{
    Json query;
    ...
    if (query_json.has_field("amount"))
    {
        query["cost"] = query_json["amount"];
    }

    return m_request->get_query_string_from_query_json(query);
}

Json APIHandlerBinanceBLVTSubscribe::handle_binance_request(Json& query_json)
{
    Json response;
    ...
    else
    {
        ...
        response["msg"] = "Subscribed BLVT token";
...
void APIHandlerBinanceBLVTSubscribe::standardize_response(Json& response)
{
    ...
    if (response.has_field("cost"))
    {
        response["amount"] = response["cost"];
    }
