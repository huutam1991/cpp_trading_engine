...
void BinanceMarketDataPerpetual::start_websocket(const Instrument* instrument)
{
    ...
    market_data->orderbook = std::make_shared<BinanceOrderBook>(instrument, 10, m_event_base);
    market_data->trade_data = std::make_shared<BinanceTradeData>(instrument, m_event_base);
