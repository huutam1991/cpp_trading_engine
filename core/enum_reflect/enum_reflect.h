...
{
#define ENUM_MAX 40  // Max number of enum values to scan
...
{
    return enum_table<E>[static_cast<int>(value)];
}
...
{
    for (std::size_t i = 0; i < enum_table<E>.size(); ++i) 
    {
        if (enum_table<E>[i] == name) 
        {
